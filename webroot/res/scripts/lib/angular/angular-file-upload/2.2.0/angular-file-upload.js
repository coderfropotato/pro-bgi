...
{
    return (typeof ($("[name=file]").attr("maxfilesize")) == "undenfined" ? 5368709120 : $("[name=file]").attr("maxfilesize"));
}
...
/***/ function (module, exports, __webpack_require__)
{
    ...
    angular.module(CONFIG.name, []).value("fileUploaderOptions", options)
        ...
        .run(
        ["FileUploader", "FileLikeObject", "FileItem", "FileDirective", "FileSelect", "FileDrop", "FileOver",
...
/***/ function (module, exports, __webpack_require__)
{
    ...
    module.exports = function (fileUploaderOptions, $rootScope, $http, $window, FileLikeObject, FileItem,$q)
    {
        ...
        var FileUploader = (function ()
        {
            ...
            _createClass(FileUploader, {
                addToQueue: {
                    ...
                    value: function addToQueue(files, options, filters)
                    {
                        ...
                        forEach(list, function (some /*{File|HTMLInputElement|Object}*/)
                        {
                            ...
                            //验证当前队列不重名，如果重名则序号加1
                            //console.info(temp);
                            var addFileName = temp.name;
                            //var index = addFileName.lastIndexOf(".");
                            //var startStr = addFileName.substring(0, index);
                            //var endStr = addFileName.substring(index);
                            //endStr = endStr.replace(/\./g, "");
                            //var count = 1;
                            var isExist = false;
                            for (var i = 0; i < _this.queue.length; i++)
                            {
                                var addedFile = _this.queue[i];
                                var addedFileName = addedFile.file.name;

                                //console.info("addFileName:" + addFileName + "  addedFileName:" + addedFileName)
                                if (addedFileName == addFileName)
                                {
                                    isExist = true;
                                    break;
                                    //addFileName = startStr + "(" + count + ")" + endStr;
                                    //count++;
                                    //temp.name = addFileName;
                                }
                            }
                            //debugger;
                            ...
                            if (_this._isValidFile(temp, arrayOfFilters, options))
                            {
                                ...
                                //fileItem.file.name = addFileName;
                                //console.info(fileItem)
                                //如果上传的文件大小大于上传文件上传限制，则取消上传
                                //如果和现有文件重名，则取消上传
                                if (fileItem.file.size > getMaxFileSize())
                                {
                                    fileItem.isOutOfLimit = true;
                                    fileItem.isUploaded = true;
                                }
                                if (isExist)
                                {
                                    fileItem.isExist = true;
                                    fileItem.isUploaded = true;
                                }
                ...
                ////高洪涛添加 2016-4-18 应用场景：批量上传时，每次上传都获取新的token
                //getUpLoadToken:
                //    {
                //        value:function()
                //        {
                ...
                //            //每次上传重新获取授权
                //            var validateData = { authorization: "GHTAO" + $window.sessionStorage.token }
                //            $http({
                //                method: "POST",
                //                data: validateData,
                //                url: options.api.base_url + "/UserLogin/iuend7d2b6a876",
                ...
                uploadAll: {
                    ...
                    value: function uploadAll()
                    {
                        ...
                        {
                            if (item.isOutOfLimit || item.isExist)
                            {
                                item.cancel();
                        ...
                        {
                            var item = items[i];
                            if (!item.isOutOfLimit && !item.isOutOfLimit)
                            {
                                item.upload();
                                break;
                            }
    ...
    module.exports.$inject = ["fileUploaderOptions", "$rootScope", "$http", "$window", "FileLikeObject", "FileItem","$q"];
