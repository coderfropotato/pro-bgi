 ...
 */
gulp.task('login:html-rev', ['login:js-rev'], function() {
    return gulp.src([modules.rev + '/rev-common-images.json', modules.rev + '/rev-common-css.json',
            modules.rev + '/rev-login-css.json', modules.rev + '/rev-login-main-js.json', modules.rev + '/rev-login-images.json'
        ].concat(login.html.main))
...
/**
 * 任务：login:js-rev
 * 作用：替换login模块中homeMain.js中引用的所有HTML和图片
 * 依赖任务：login:amd，必须保证homeMain.js已执行AMD导入
 */
gulp.task('login:js-rev', ['login:amd'], function() {
    return gulp.src([modules.rev + '/rev-login-html.json', login.js.main.replace('webroot', argv.build)])
        .pipe(revCollector({ replaceReved: true }))
        .pipe(uglify())
        .pipe(rev())
 ...
 */
gulp.task('login:amd', ['login:html'], function() {
...
/**
 * 任务：login:html
 * 作用：提供login模块非主页面的所有页面中图片和CSS路径，并对页面进行压缩
 * 依赖任务：clean:login，清理文件夹
 */
gulp.task('login:html', ['clean:login'], function() {
    return gulp.src([modules.rev + '/rev-login-images.json'].concat(login.html.src))
        .pipe(revCollector({ replaceReved: true }))
        .pipe(minifyHTML({
            empty: true,
            spare: true
        ...
        .pipe(gulp.dest(argv.build + login.html.dest))
        .pipe(rev.manifest({ merge: true }))
        .pipe(rename('rev-login-html.json'))
        .pipe(gulp.dest(modules.rev));
})
