...
define("superApp.addDeleteBigTableDnaDire", ["angular", "super.superMessage", "select2"],
    function(angular, SUPER_CONSOLE_MESSAGE) {
        ...
        function addDeletBigTableCtr($rootScope, $scope, $log, $state, $window, $timeout, ajaxService, toolService, reportService, pageFactory) {
            ...
            // thead change event
            // add track by id start flag=> add trackById
            // add trackById
            $scope.curAdd = [];
            $scope.curDelete = [];
            ...
            $scope.theadChange = function(a) {
                ...
                a.add.forEach(function(val, index) {
                        ...
                        // add trackById
                        $scope.curAdd = $scope.curAdd.concat(val.children);
                    })
                    // 所有新加的表头
                ...
                a.delete.forEach(function(val, index) {
                    ...
                    // add trackById
                    $scope.curDelete = $scope.curDelete.concat(val.children);
                });
            ...
            $scope.GetBigTableData = function(pageNumber) {
                ...
                promise.then(function(responseData) {
                    if (responseData.Error) {
                        ...
                        if (!$scope.curAdd.length && !$scope.curDelete.length) {
                            $scope.idArray.length = $scope.bigTableData.baseThead.length;
                        } else {
                            // 有新增或者删除
                            if ($scope.curAdd.length) {
                                for (var i = $scope.baseLength; i < $scope.bigTableData.baseThead.length; i++) {
                                    for (var m = 0; m < $scope.curAdd.length; m++) {
                                        if ($scope.bigTableData.baseThead[i].true_key === $scope.curAdd[m]) {

                                            $scope.idArray.splice(i, 0, Math.random());
                                            break;
                                        }
                                    }
                                }
                            ...
                            if ($scope.curDelete.length) {
                                for (var j = $scope.baseLength; j < $scope.beforeBigData.baseThead.length; j++) {
                                    for (var k = 0; k < $scope.curDelete.length; k++) {
                                        if ($scope.beforeBigData.baseThead[j].true_key === $scope.curDelete[k]) {
                                            $scope.idArray.splice(j, 1);
                                            break;
                                        }
                                    }
                                }
                        ...
                        // 每次加完id清除  防止下一次不是增删列获取表格数据的时候重新应用上一次的新增和删除，那么上一次新增和删除的列会产生新的id，
                        ...
                        $scope.beforeBigData = angular.copy($scope.bigTableData);
                        // add track by id over
