...
define("superApp.reAnalysisDire", ["angular", "super.superMessage", "select2"],
    function(angular, SUPER_CONSOLE_MESSAGE) {
        ...
        function reAnalysisDirective($log) {
            return {
                ...
                template: "<div class='re-analysis-panel clearfix'>" +
                    ...
                    "<li ng-if='netShow' class='pull-left' ng-class=\"{'disabled':netError}\" uib-tooltip=\"{{netError?'选择1-500个基因作图':'蛋白网络图'}}\" ng-click=\"handlerReanalysisClick(netError,'net')\"><i class=\"iconfont icon-tools icon-danbaiwangluohuzuo\"></i></li>" +
                    "</ul>" +
                    "</div>",
                scope: {
                    callback: "&",
        ...
        function reAnalysisCtr($rootScope, $scope, $log, $state, $window, $timeout, ajaxService, toolService, reportService) {
            ...
            // 小工具配置显示
            if (toolService.sessionStorage.get('toolsInfo') !== 'undefined') {
                $scope.toolsList = JSON.parse(toolService.sessionStorage.get('toolsInfo'));
                $scope.toolsList.forEach(function(val, index) {
                    $scope[val + 'Show'] = true;
                })
            } else {
                $scope.toolsList = [];
