    ...
    */
    bodyFormatter: function (items) {
        return map(items, function (item) {
            ...
            /* 160219 加入 */
            if (item.point.low || item.point.high) {
                if (item.point.low < item.point.high) {
                    item.point.startDate = chartGetDateTime(item.point.low);
                    item.point.endDate = chartGetDateTime(item.point.high);
                } else {
                    item.point.startDate = chartGetDateTime(item.point.high);
                    item.point.endDate = chartGetDateTime(item.point.low);
                }
            }
